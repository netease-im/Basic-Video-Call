...
appKeyStr = re.search('let\skAppKey\s?=\s?\".*\"',text).group()
if re.match("let kAppKey\s?=\s?\"[a-f0-9]{32}\"", appKeyStr) is not None:
...
ret = re.sub('let\skAppKey\s?=\s?\".*\"', 'let kAppKey = \"{}\"'.format(APP_KEY), text)
io.seek(0)
io.write(ret)
io.truncate()
