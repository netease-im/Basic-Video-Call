...
struct NERtcTextWatermarkConfig {
     ...
     * The text content.
     * @note
     * - The length of the string is unlimited. The final display is affected by the font size and the size of the
     * watermark frame. The part beyond the watermark frame is not displayed.
     * - If the width of the watermark frame is set, when the length of the text content exceeds the width of the
     * watermark frame, the text automatically wraps. If the text exceeds the height of the watermark frame, the excess
     * part is not displayed.
     * - When the width and height of the watermark frame are not set, the text does not wrap, and the part beyond the
     * watermark frame is not be displayed.
     * @endif
     * @if Chinese
     * 文字内容。
     * @note
    ...
    /**
     * @if English
     * The horizontal distance between the upper left corner of the watermark and the upper left corner of the video
     * canvas. Unit: pixels.
     * @endif
     * @if Chinese
    ...
    /**
     * @if English
     * The vertical distance between the upper left corner of the watermark and the upper left corner of the video
     * canvas. Unit: pixels.
     * @endif
     * @if Chinese
...
struct NERtcImageWatermarkConfig {
     ...
     * @if English
     * The horizontal distance between the upper left corner of the watermark and the upper left corner of the video
     * canvas. Unit: pixels. The default value is 0.
     * @endif
     * @if Chinese
    ...
    /**
     * @if English
     * The width of the watermark image. Unit: pixels. The default value is 0, which indicates that the width of the
     * original image is applied.
     * @endif
     * @if Chinese
    ...
    /**
     * @if English
     * The height of the watermark image. Unit: pixels. The default value is 0, which indicates that the height of the
     * original image is applied.
     * @endif
     * @if Chinese
    ...
    /**
     * @if English
     * The frame rate. The default value is 0 fps, which indicates that the images are not flipped automatically. Images
     * are displayed in static single frames. Note: The frame rate for clients on Windows does not exceed 20 fps.
     * @endif
     * @if Chinese
    ...
    /**
     * @if English
     * Specifies whether to loop. By default, loop is enabled. If the value is set to false, the watermarks disappear
    ...
    NERtcImageWatermarkConfig() : offset_x(0), offset_y(0), image_width(0), image_height(0), fps(0), loop(true) {
 ...
 * @note You can set text, timestamp, and image watermarks at the same time. If different types of watermarks overlay,
 * the layers overlay previous layers in the image, text, and timestamp sequence.
 * @endif
 * @if Chinese
 * 画布水印设置。
...
struct NERtcCanvasWatermarkConfig {
    ...
    NERtcCanvasWatermarkConfig()
        : image_watermarks(NULL), image_count_(0), text_watermarks(NULL), text_count_(0), timestamp_watermark(NULL) {}
};
 ...
 * 3D音效算法中坐标信息。 
 */
struct NERtcSpatializerPositionInfo {
  /**
