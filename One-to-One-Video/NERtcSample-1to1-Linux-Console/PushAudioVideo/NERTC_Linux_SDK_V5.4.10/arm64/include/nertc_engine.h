...
public:
     ...
     * @par 使用前提
     * 请先通过 \ref nertc::IRtcEngine::setChannelProfile "setChannelProfile" 接口设置房间模式为直播模式，并通过 \ref nertc::IRtcEngine::setClientRole "setClientRole" 接口设置房间成员的角色为观众。
     * @par 调用时机
     * 请在引擎初始化之后调用此接口，且该方法仅可在加入房间后调用。
     * @note
     * 房间成员成功切换房间后，默认订阅房间内所有其他成员的音频流，因此会产生用量并产生计费；若想取消订阅，可以调用 \ref nertc::IRtcEngineEx::subscribeRemoteAudioStream "subscribeRemoteAudioStream" 方法，并设置 subscribe 参数为 false。
     * @par 参数说明
     * <table>
     *  <tr>
     *      <th>**参数名称**</th>
