...
public:
    ...
    */
     ...
     * 请在引擎初始化之后调用此接口，且该方法仅可在加入房间后调用。
     * @note
     * - 该方法仅适用于 Windows 平台；若您使用的是 macOS 平台，请调用 \ref nertc::IRtcEngineEx::startScreenCaptureByDisplayId "startScreenCaptureByDisplayId" 方法。
     ...
     * 请在引擎初始化之后调用此接口，且该方法仅可在加入房间后调用。
     * @note
     * 该方法仅适用于 macOS 平台；若您使用的是 Windows 平台，请调用 \ref nertc::IRtcEngineEx::startScreenCaptureByScreenRect "startScreenCaptureByScreenRect" 方法。
     * @par 参数说明
     * <table>
     *  <tr>
     *      <th>**参数名称**</th>
   ...
   * - 其他: 调用失败
   */
  virtual int updateSpatializerAudioRecvRange(int audible_distance,
                                              int conversational_distance,
                                              NERtcDistanceRolloffModel roll_off) = 0;
  ...
  /**
    ...
    * @note 
    * - 请先调用 \ref  #enableSpatializer 接口启用空间音效，再调用本接口。
    * - 调用 \ref  #enableSpatializer 接口关闭空间音效后，空间位置信息会重置为默认值，您需要重新设置位置信息。
    * @par 参数说明
    * 通过 info 参数设置空间音效中说话者和接收者的空间位置信息。 \ref nertc::NERtcSpatializerPositionInfo "NERtcSpatializerPositionInfo" 的具体参数说明如下表所示。
    * <table>
    *  <tr>
    *      <th>**参数名称**</th>
    *      <th>**描述**</th>
