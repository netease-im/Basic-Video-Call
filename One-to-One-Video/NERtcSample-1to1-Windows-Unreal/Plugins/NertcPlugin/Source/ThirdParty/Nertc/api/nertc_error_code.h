...
typedef enum {
  ...
  */
  ...
  kNERtcErrSEIExceedMaxDataLimit = 30122,      /** SEI 超过最大数据限制 */
  /**
   * @if Chinese
   * 数据通道发送大小超过最大限制
   * @endif
...
} nertc_room_server_error_code;

/** @enum nertc_audio_mixing_error_code 混音音乐文件错误码。
*/
typedef enum {
  /**
   * @if English
...
} nertc_audio_mixing_error_code;


#endif //NERTC_ERROR_CODE_H
