...
typedef enum
{
    kNERtcAudioScenarioDefault  = 0,    /** 0: 默认设置为 kNERtcAudioScenarioSpeech。 */
...
struct NERtcStats
{
    uint64_t tx_bytes;	/**< 发送字节数（KB），累计值。*/
    uint64_t rx_bytes;	/**< 接收字节数（KB），累计值。*/
    double cpu_app_usage;   /**< 当前 App 的 CPU 使用率 (%)。*/
    double cpu_idle_usage;  /**< 当前系统的 CPU 空闲率 (%)。*/
    double cpu_total_usage;	/**< 当前系统的 CPU 使用率 (%)。*/
};
...
struct NERtcScreenCaptureParameters
{
    ...
    NERtcVideoDimensions dimensions;    /**< 屏幕共享视频发送的最大像素值。*/
    int frame_rate;                     /**< 共享视频的帧率，单位为 fps；默认值为 5，建议不要超过 15。*/
...
//#define kNERtcKeyAutoStartLocalAudio        "auto_start_local_audio"       /**< bool value，加入频道自动打开自己的音频。默认值 true */
//#define kNERtcKeyAutoStartLocalVideo        "auto_start_local_video"       /**< bool value，加入频道自动打开自己的视频。默认值 true */
#define kNERtcKeyAutoSubscribeAudio         "auto_subscribe_audio"         /**< bool value，其他用户打开音频时，自动订阅。 默认值 true */
//#define kNERtcKeyAutoSubscribeVideo         "auto_subscribe_video"         /**< bool value，其他用户打开视频时，自动订阅。默认值 true */
//#define kNERtcKeyVideoCaptureObserverEnabled "video_capture_observer_enabled"   /**< bool value，视频数据采集回调，开发者可以获取到原始视频数据。默认值 false */
